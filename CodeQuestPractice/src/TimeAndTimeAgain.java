		...
		for (int j = 0; j < numberOfIterations; j++) {
			...
			while (time[0] != -1 || time[1] != -1 || time[2] != -1) {
				...
				try {
					...
					timeStrings[i] = String.format("%02d", Integer.parseInt(line.substring(toTry - 2, toTry)))
							+ (i == 2 ? "" : ":");
				} catch (IndexOutOfBoundsException | NumberFormatException ex) {
					timeStrings[i] = String.format("%02d", Integer.parseInt(line.substring(toTry - 1, toTry)))
							+ (i == 2 ? "" : ":");
				}
				// Create strings for blank time increments
				if (counter == 0 && time[0] == -1)
					timeStrings[0] = "00:";
				if (counter == 0 && time[1] == -1)
					timeStrings[1] = "00:";
				if (counter == 0 && time[2] == -1)
					timeStrings[2] = "00";
				time[i] = -1;
				counter++;
			}
